    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            <a href="{{ route('admin.users') }}"
                class="inline-block mb-6 px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300">&larr; Back to
                Users</a>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-6 flex flex-col gap-2 md:row-span-2 md:h-full"
                    ...
                    <div><span class="font-semibold">Joined:</span>
                        {{ $user->created_at ? $user->created_at->format('Y-m-d') : 'N/A' }}</div>
                    <div><span class="font-semibold">Status:</span> <span
                            class="{{ $user->is_suspended ? 'text-red-600' : 'text-green-600' }} font-semibold">{{ $user->is_suspended ? 'Suspended' : 'Active' }}</span>
                    </div>
                    <div><span class="font-semibold">Last Logged In:</span>
                        {{ $user->last_login_at ? $user->last_login_at->diffForHumans() : 'N/A' }}</div>
                    <div class="flex gap-2 mt-4">
                        <form action="{{ route('admin.users.suspend', $user) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="px-3 py-1 rounded {{ !$user->is_suspended ? 'bg-yellow-500 text-white' : 'bg-green-600 text-white' }}">
                        ...
                        <form action="{{ route('admin.users.delete', $user) }}" method="POST"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="px-3 py-1 rounded bg-red-600 text-white">Delete
                                Account</button>
                        </form>
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse($recentTransactions as $txn)
                                <tr>
                                    ...
                                    <td class="py-2 px-4">
                                        {{ $txn->date ? \Carbon\Carbon::parse($txn->date)->diffForHumans() : 'N/A' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="py-2 px-4 text-center text-gray-400">No recent activity</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
